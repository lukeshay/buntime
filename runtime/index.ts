...
const log = (...args: any[]) => buntimeDebug && console.log("[Buntime]", ...args);
...
const invocationUrl = `http://${Bun.env.AWS_LAMBDA_RUNTIME_API!}/2018-06-01/runtime/invocation`;
...
const buildContext = (next: Response, requestId: string): Context => {
	...
	return {
		...
		invokedFunctionArn: String(next.headers.get("lambda-runtime-invoked-function-arn")),
